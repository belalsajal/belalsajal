Hi there, I'm Belal 👋
🎓 Data Scientist | AI Enthusiast | Problem Solver

Welcome to my GitHub! I developed a strong foundation in Python programming, machine learning, data engineering, and deep learning. I’m passionate about using data to solve real-world problems, especially when it comes to making an impact through AI and machine learning.

🚀 My Latest Project: Waste Classification Using Deep Learning
One of the highlights of my Bootcamp was building an end-to-end waste classification system using Convolutional Neural Networks (CNN). I used the powerful ResNet-50 model, working with 12 waste categories and achieving an impressive 94% accuracy. This project was a fantastic opportunity to see how AI can contribute to sustainability by helping to automate waste sorting. 🌍♻️

🔗 Check it out: [https://github.com/belalsajal/TreasureBot3000]

🛠️ Technologies & Tools I Use:
Programming: Python, SQL
Libraries: TensorFlow, PyTorch, Pandas, NumPy, Scikit-learn
Data Visualization: Matplotlib, Seaborn, Plotly
Machine Learning: Supervised & Unsupervised Learning, Deep Learning (CNN, RNN, NLP)
Data Engineering: Data Wrangling, ETL, APIs
Cloud: Google Cloud, AWS
Version Control: Git, GitHub
👩‍💻 Currently Exploring:
Advanced Natural Language Processing (NLP)
Optimizing Deep Learning models for real-world use cases
Exploring the intersection of AI and sustainability
🌟 Let’s Collaborate!
I'm always open to discussing new projects, especially those involving machine learning, AI, or data-driven innovation. If you’re interested in collaborating or simply want to chat about data science, feel free to reach out!

📫 How to reach me:

LinkedIn: https://www.linkedin.com/in/belal-sajal/
Email: belal.sajal@gmail.com
